...
class WidgetValidator(widget: Widget) extends ValidationGroup(new ClasspathMessageResolver(classOf[WidgetValidator])){
  this.add(new NotNullOrNoneValidator("name", {widget.name}))
  this.add(new NotNullOrNoneValidator("id", {widget.id}))
  this.add(new NotNullOrNoneValidator("Year Made", {widget.yearMade}))
  this.add(new StringLengthValidator("name", 5, 50, {widget.name}))
