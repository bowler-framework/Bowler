...
class SelectorsTest extends FunSuite{
  test("defaultLayoutSelector"){
    ...
    assert(selector.layout(makeRequest(Map())).get.name == "default")
  }
  ...
  test("headerContainsLayoutSelector"){
    ...
    assert(selector.layout(makeRequest(Map())) == None)
  ...
  test("uri matcher selector"){
    ...
    assert(selector.layout(new DummyRequest(HTTP.POST, "/hello/world", Map(), null)).get.name == "default")
  }

  test("uri & method matcher selector"){
    val selector = new UriAndMethodLayoutSelector(Layout("default"), HTTP.GET, new Regex("^.*/hello/.*$"))
    assert(selector.layout(new DummyRequest(HTTP.POST, "/hello/world", Map(), null)) == None)

    assert(selector.layout(new DummyRequest(HTTP.GET, "/hello/world", Map(), null)).get.name == "default")
  }
